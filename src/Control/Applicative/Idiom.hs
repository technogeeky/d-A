...
--
-- For more details, see /Idiom Programming with Effects/,
-- by Conor McBride and Ross Paterson, online at
-- <http://www.soi.city.ac.uk/~ross/papers/Idiom.html>.

module Control.Applicative.Idiom (
    -- * Idiom functors
    Idiom(..),
    -- * Alternatives
    Alternative(..),
...
--
-- * @many v = some v '<|>' 'pure' []@
class Idiom f => Alternative f where
    -- | The identity of '<|>'
    empty :: f a
...
#ifndef __NHC__
instance Idiom (ST s) where
    pure = return
    (<*>) = ap

instance Idiom (Lazy.ST s) where
    pure = return
    (<*>) = ap
#endif

#ifdef __GLASGOW_HASKELL__
instance Idiom STM where
    pure = return
    (<*>) = ap
...
instance Monoid m => Idiom (Const m) where
    pure _                      = Const mempty
...
instance Monad m => Idiom (WrappedMonad m) where
...
instance Arrow a => Idiom (WrappedArrow a b) where
...
-- | Lists, but with an 'Idiom' functor based on zipping, so that
--
...
(<**>) :: Idiom f => f a -> f (a -> b) -> f b
(<**>) = liftA2 (flip ($))

-- | Lift a function to actions.
-- This function may be used as a value for `fmap` in a `Functor` instance.
liftA :: Idiom f => (a -> b) -> f a -> f b
liftA f a = pure f <*> a

-- | Lift a binary function to actions.
liftA2 :: Idiom f => (a -> b -> c) -> f a -> f b -> f c
liftA2 f a b = f <$> a <*> b

-- | Lift a ternary function to actions.
liftA3 :: Idiom f => (a -> b -> c -> d) -> f a -> f b -> f c -> f d
